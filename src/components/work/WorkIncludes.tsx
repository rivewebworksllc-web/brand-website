...
export function WorkIncludes({ items }: { items: WorkInclude[] }) {
  const cycle = usePresentationCycle({ itemCount: items.length, interval: 7000 });
  const active = items[cycle.activeIndex] ?? items[0];

  return (
    <div ref={cycle.containerRef} {...cycle.interactionProps} data-presentation-mode={cycle.mode} className="grid gap-10 lg:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)] lg:gap-16">
          ...
          return (
            <button
              ...
              onKeyDown={(event) => {
                ...
                cycle.select(items.indexOf(next));
                document.getElementById(`work-include-tab-${next.id}`)?.focus();
              }}
              className={`group relative grid min-h-20 w-full grid-cols-[2rem_1fr_auto] items-center gap-3 py-4 text-left transition-colors duration-300 motion-reduce:transition-none ${selected ? "text-heading" : "text-muted hover:text-heading"}`}
            >
