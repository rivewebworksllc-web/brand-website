...
export function ResourceCard({ category, title, summary, href, metaStatus }: ResourceCardContent) {
  ...
  const classes = "card group flex h-full flex-col p-6";
  return isAvailableHref(href) ? <Link href={href} className={`${classes} card-interactive`}>{content}</Link> : <article className={classes}>{content}</article>;
}
